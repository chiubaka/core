...
function ormReducer(orm) {
    return (database = orm.getEmptyState(), action) => {
        ...
        switch (action.type) {
            ...
            case actions_1.ModelActionType.SUCCESSFUL_SEARCH_MODEL: {
                successfulListModel(model, action);
                break;
            }
            case actions_1.ModelActionType.SUCCESSFUL_GET_MODEL: {
                successfulGetModel(model, action);
                break;
            }
...
function successfulGetModel(model, action) {
    const payload = action.payload;
    model.upsert(payload);
}
